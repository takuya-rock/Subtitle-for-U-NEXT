...
const setupUI = () => {
    ...
    fileInput.onchange = (e) => {
        ...
        // 1. ファイル名とIDをセット
        fileName = file.name; 
        subId = "local_" + file.name; 
        
        const label = document.getElementById("file-name-label");
        if (label) {
            label.innerText = "📄 " + fileName;
            label.style.color = "#00ffcc"; // 読み込み成功の色
        }

        const reader = new FileReader();
        reader.onload = (ev) => {
            // 2. 字幕データを解析して変数に格納
            srtContent = ev.target.result;
            subtitles = parseSRT(srtContent);
            
            // 3. ストレージに保存
            saveToStorage();

            // 4. 【重要】即座に画面とプレビューを更新する
            if (window.updateSubtitleImmediately) {
                window.updateSubtitleImmediately();
            }
            if (window.updateManagerPreview) {
                window.updateManagerPreview();
            }

            // 5. 同じファイルを再度選べるように値をリセットしておく
            fileInput.value = "";
        };
